...
<xsl:template name="english_rank">
<xsl:param name="rank"/>
<xsl:value-of name="color" select="$translations/trans:card[@id=$rank]"/>
</xsl:template>

<xsl:template name="english_denom">
<xsl:param name="denom"/>
<xsl:param name="level" select="1"/>
<xsl:value-of select="$translations/trans:denom[@id=$denom]"/>
<xsl:if test="$level>1">
<xsl:value-of select="$translations/trans:denom[@id=$denom]/@plural"/>
</xsl:if>
</xsl:template>

<xsl:template name="english_holding">
<xsl:param name="holding"/>
</xsl:template>

<xsl:template name="english_card">
<xsl:param name="suit"/>
<xsl:param name="rank"/>
    <xsl:call-template name="english_denom">
        <xsl:with-param name="denom" select="$suit"/>
    </xsl:call-template>
    <xsl:text> </xsl:text> 
    <xsl:call-template name="english_rank">
...
<xsl:template match="bridge:club">
    <xsl:call-template name="suit">
        ...
        <xsl:with-param name="english" select="'club'"/>
    </xsl:call-template>
</xsl:template>
...
<xsl:template match="bridge:diamond">
    <xsl:call-template name="suit">
        ...
        <xsl:with-param name="english" select="'diamond'"/>
    </xsl:call-template>
</xsl:template>
...
<xsl:template match="bridge:heart">
    <xsl:call-template name="suit">
        ...
        <xsl:with-param name="english" select="'heart'"/>
    </xsl:call-template>
</xsl:template>
...
<xsl:template match="bridge:spade">
    <xsl:call-template name="suit">
        ...
        <xsl:with-param name="english" select="'spade'"/>
    </xsl:call-template>
</xsl:template>
...
<xsl:when test="@card">
<xsl:variable name="english_card">
   <xsl:value-of select='$english'/>
   <xsl:text> </xsl:text>
   <xsl:call-template name="english_rank">
       <xsl:with-param name='rank' select='@card'/>
   </xsl:call-template>
</xsl:variable>
<abbr class='card' title='{$english_card}'>
<xsl:call-template name="holding">
...
<xsl:with-param name="symbol" select="$symbol"/>
<xsl:with-param name="separator" select="'-'"/>
<xsl:with-param name="english" select="$english"/>
</xsl:call-template>
</xsl:when>
<xsl:otherwise>
...
<xsl:template name="english_contract">
<xsl:param name="code"/>
<xsl:param name="declarer"/>
    ...
    <xsl:variable name="englishDenom">
       <xsl:call-template name="english_denom">
    ...
    <xsl:variable name="englishLevel">
       <xsl:choose>
           <xsl:when test="$translations/trans:level[@id=$level]">
              <xsl:value-of select="$translations/trans:level[@id=$level]"/>
           </xsl:when>
           <xsl:otherwise>
    ...
    <xsl:value-of select="$englishLevel"/>
    <xsl:text> </xsl:text>
    <xsl:value-of select="$englishDenom"/>

    <xsl:if test="$rest">
        <xsl:text> </xsl:text>
        <xsl:choose>
           <xsl:when test="$translations/trans:contractModifier[@id=$rest]">
              <xsl:value-of select="$translations/trans:contractModifier[@id=$rest]"/>
           </xsl:when>
           <xsl:otherwise>
    ...
    <xsl:if test="$declarer">
      ...
      <xsl:value-of select="$translations/trans:seatname[$declarer=@id][1]/@ucase"/>
    </xsl:if>
...
<xsl:param name="code"/>
    ...
    <xsl:variable name="english">
        <xsl:call-template name="english_contract">
            <xsl:with-param name="code" select="@code"/>
            <xsl:with-param name="declarer" select="@declarer"/>
        </xsl:call-template>
    </xsl:variable>

<abbr class="contract" title="{$english}">
<xsl:value-of select="$level"/>
<xsl:text> </xsl:text>
<xsl:copy-of select="$symbol"/>
<xsl:if test="$rest">
